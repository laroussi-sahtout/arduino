...
#include <HCSR04.h> // Inclure la bibliothèque HCSR04
...
#define MAX_DISTANCE 20 // Distance maximale à laquelle on affiche "Salut" en cm
...
HCSR04 sensor(A1, A2); // Définir les broches de déclenchement et d'écho du capteur HCSR04
...
void loop() {
  ...
  float distance = sensor.dist(); // Mesurer la distance avec le capteur HCSR04

  if (distance < MAX_DISTANCE) { // Si la distance est inférieure à la distance maximale
    displayMessage("Salut"); // Afficher "Salut" sur l'écran OLED
    while (true) {} // Arrêter toutes les autres opérations
  }
